...
mouse_tracking = True  # Flag to control mouse tracking
...
class TransparentOverlay(tk.Tk):
    ...
    def update_position(self):
        x, y = pyautogui.position()
        # Position window above the mouse cursor
        self.geometry(f"{self.window_width}x{self.window_height}+{x - self.window_width // 2}+{y - self.window_height - self.offset_y}")
...
def update_overlay_position():
    """Thread function to continuously update the overlay position"""
    global overlay_window, stop_event
    
    while not stop_event.is_set() and overlay_window:
        try:
            overlay_window.update_position()
...
def run_vosk():
    ...
    """
    global overlay_window, position_update_thread
    ...
    position_update_thread = threading.Thread(target=update_overlay_position, daemon=True)
